    ...
    try {
      ...
      for (const placeVisit of placeVisits) {
        try {
          ...
          let enrichedName: string | null = placeVisit.name ?? null;
          let enrichedAddress: string | undefined = placeVisit.address;
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          let googlePlacesMetadata: Record<string, any> | null = null;

          // TIER 0: Check own DB for Place ID match (fastest path)
          if (placeVisit.place_id) {
            ...
            if (existingVenue) {
              ...
              // Create visit directly for existing venue
              ...
              continue; // Skip to next place visit
            }
          }
          ...
          if (!matchResult.venue) {
            visits_skipped++;
            errors.push({
              place_name: placeVisit.name ?? `Coordinates: ${placeVisit.latitude}, ${placeVisit.longitude}`,
          ...
          this.logger.error(`Error processing place visit "${placeVisit.name ?? 'coordinate-only entry'}": ${error}`);
          visits_skipped++;
          errors.push({
            place_name: placeVisit.name ?? `Coordinates: ${placeVisit.latitude}, ${placeVisit.longitude}`,
            address: placeVisit.address,
