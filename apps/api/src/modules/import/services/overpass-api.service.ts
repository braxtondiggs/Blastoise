...
// For venue discovery (map feature)
...
interface OverpassElement {
  ...
  lat?: number;
  lon?: number;
  center?: { lat: number; lon: number }; // For way/relation elements
  tags?: {
    name?: string;
    craft?: string;
    amenity?: string;
...
export class OverpassApiService {
  ...
  // ============================================
  // VENUE DISCOVERY (for map feature)
  // ============================================

  /**
   * Discover breweries and wineries near a location using Overpass API
   * Used by the map feature for venue discovery
  ...
  async discoverNearby(
    ...
    try {
      // Check cache first
      const cacheKey = `osm-discovery:${latitude.toFixed(4)},${longitude.toFixed(4)}:${radiusMeters}`;
      const cached = await this.cacheService.get<OsmVenueResult[]>(cacheKey);
      if (cached) {
        this.logger.debug(`Cache hit for OSM discovery: ${cacheKey}`);
        return cached;
      }
      ...
      const venues = await this.limiter.schedule(() =>
        this.fetchDiscoveryResults(latitude, longitude, radiusMeters)
      );

      // Cache results (1 hour TTL - venues don't change often)
      if (venues.length > 0) {
        await this.cacheService.set(cacheKey, venues, 60 * 60);
      }
  ...
  /**
   * Build and execute Overpass query for venue discovery
   */
  private async fetchDiscoveryResults(
    latitude: number,
    longitude: number,
    radiusMeters: number
  ): Promise<OsmVenueResult[]> {
    // Overpass QL query for breweries and wineries (broader than verification query)
    const query = `
      [out:json][timeout:25];
      (
        // Breweries
    ...
    try {
      ...
      this.logger.error(`Overpass API discovery request failed: ${error}`);
      return [];
    }
  }

  /**
   * Parse Overpass response into venue discovery results
   */
  private parseDiscoveryResponse(data: OverpassResponse): OsmVenueResult[] {
